...
exports.MonarchClient = void 0;
const auth_1 = require("./auth");
const DirectAuthenticationService_1 = require("./auth/DirectAuthenticationService");
...
const cashflow_1 = require("../api/cashflow");
const recurring_1 = require("../api/recurring");
const institutions_1 = require("../api/institutions");
const insights_1 = require("../api/insights");
const investments_1 = require("../api/investments");
...
class MonarchClient {
    constructor(config = {}) {
        ...
        if (this.config.enablePersistentCache) {
            this.cache = new cache_1.MultiLevelCache(this.config.cache, this.config.cacheEncryptionKey);
        }
        // Initialize authentication service
        this.auth = new auth_1.AuthenticationService(this.config.baseURL, undefined);
        ...
        this.directAuth = new DirectAuthenticationService_1.DirectAuthenticationService(this.config.baseURL, this.auth.sessionStorage // Access private property for shared storage
        );
        // Initialize GraphQL client
        this.graphql = new graphql_1.GraphQLClient(this.config.baseURL, this.auth, this.cache, this.config.timeout);
        // Initialize API modules
        this.accounts = new accounts_1.AccountsAPIImpl(this.graphql);
        this.transactions = new transactions_1.TransactionsAPIImpl(this.graphql);
        this.budgets = new budgets_1.BudgetsAPIImpl(this.graphql);
        this.categories = new categories_1.CategoriesAPIImpl(this.graphql);
        this.cashflow = new cashflow_1.CashflowAPIImpl(this.graphql);
        this.recurring = new recurring_1.RecurringAPIImpl(this.graphql);
        this.institutions = new institutions_1.InstitutionsAPIImpl(this.graphql);
        this.insights = new insights_1.InsightsAPIImpl(this.graphql);
        this.investments = new investments_1.InvestmentsAPIImpl(this.graphql);
        this.reports = new reports_1.ReportsAPIImpl(this.graphql);
        utils_1.logger.info('MonarchClient initialized', {
    ...
    buildConfig(userConfig) {
        ...
        const envConfig = {
            email: (0, utils_1.getEnvironmentVariable)('MONARCH_EMAIL'),
            password: (0, utils_1.getEnvironmentVariable)('MONARCH_PASSWORD'),
            sessionToken: (0, utils_1.getEnvironmentVariable)('MONARCH_SESSION_TOKEN'),
            baseURL: (0, utils_1.getEnvironmentVariable)('MONARCH_BASE_URL'),
            cacheEncryptionKey: (0, utils_1.getEnvironmentVariable)('MONARCH_CACHE_ENCRYPTION_KEY'),
            logLevel: (0, utils_1.getEnvironmentVariable)('MONARCH_LOG_LEVEL'),
        };
        ...
        config = (0, utils_1.deepMerge)(config, cleanedEnvConfig);
        config = (0, utils_1.deepMerge)(config, userConfig);
        return config;
    }
    ...
    setTimeout(timeoutMs) {
        ...
        utils_1.logger.info(`Timeout updated to ${timeoutMs}ms`);
    }
    setToken(token) {
        this.config.sessionToken = token;
        // Note: This would need to be passed to the auth service
        utils_1.logger.info('Token updated');
    }
    // Cleanup
    async close() {
        this.cache?.close();
        utils_1.logger.info('MonarchClient closed');
    }
...
exports.MonarchClient = MonarchClient;
