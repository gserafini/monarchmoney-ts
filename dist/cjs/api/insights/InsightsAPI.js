...
class InsightsAPIImpl {
    ...
    async getAdviceItems(group = 'objective') {
        ...
        const validGroup = VALID_ADVICE_GROUPS.includes(group)
            ...
            : 'objective';
        if (group !== validGroup) {
            utils_1.logger.warn('Invalid advice group provided, defaulting to "objective"', { provided: group });
        }
        const query = `
    ...
    `;
        try {
            ...
            utils_1.logger.error('Failed to get advice items:', { error });
            return [];
        }
    }
    /**
    ...
    `;
        try {
            ...
            utils_1.logger.error('Failed to get spinwheel user:', { error });
            return null;
        }
    }
    /**
    ...
    `;
        try {
            ...
            utils_1.logger.error('Failed to get weekly recap:', { error });
            return null;
        }
    }
    /**
    ...
    `;
        try {
            ...
            if (status !== 400) {
                utils_1.logger.error('Failed to get credit score snapshots:', { error });
            }
            return [];
        }
    }
    ...
    async dismissInsight(_insightId) {
        utils_1.logger.warn('dismissInsight is not available via the Monarch API');
        return false;
    }
}
exports.InsightsAPIImpl = InsightsAPIImpl;
