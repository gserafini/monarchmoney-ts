...
import { validateLoginCredentials, validateMFACredentials, logger, MonarchAuthError, MonarchMFARequiredError, MonarchCaptchaRequiredError, MonarchIPBlockedError, MonarchSessionExpiredError, handleHTTPResponse, retryWithBackoff, EncryptionService } from '../../utils';
    ...
    async performLoginWithMFA(email, password, mfaSecretKey) {
        ...
        if (response.status >= 400) {
            ...
            if (data.error_code === 'CAPTCHA_REQUIRED' || data.detail?.includes('CAPTCHA')) {
                throw new MonarchCaptchaRequiredError('CAPTCHA verification required. Please log in through the web interface first to clear this requirement.');
            }
            ...
            if (data.You === 'Shall Not Pass' || data.detail?.includes('Shall Not Pass') ||
                ...
                throw new MonarchIPBlockedError('Your IP address has been temporarily blocked. Please wait some time before trying again, or try from a different network/IP address.');
            }
